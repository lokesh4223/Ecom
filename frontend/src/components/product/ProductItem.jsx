...
const ProductItem = ({ product, isItemOnBasket, addToBasket }) => {
  ...
  const handleAddToBasket = () => {
    if (addToBasket) {
      // Ensure product has proper default values when adding to basket
      const productToAdd = {
        ...product,
        selectedSize: product.sizes && product.sizes[0] ? product.sizes[0] : 'M',
        quantity: product.quantity || 1,
        selectedColor: product.selectedColor || (product.availableColors && product.availableColors[0]) || '#000000'
      };
