...
const ProductList = (props) => {
  ...
  // Check if we have products but filtering resulted in none
  if (products.items.length > 0 && filteredProducts.length === 0 && !isLoading) {
    return (
      <MessageDisplay message={requestStatus?.message || 'No products match the current filters.'} />
    );
  }

  // Check if we have no products at all
  if (products.items.length === 0 && !isLoading && !requestStatus) {
    return (
      <MessageDisplay message="No products found." />
    );
  }

  if (products.items.length === 0 && requestStatus && !isLoading) {
    return (
      <MessageDisplay
  ...
  return (
    <Boundary>
      ...
      {(products.total > 0 && products.items.length < products.total) && (
